...
//    y[30] = 1.0f - (y[31] + y[32] + y[33] + y[34]);
  ...
  double m11_inf = pow(1.0/(1.0 + exp(-1.0 * (y[1] + 31.0) / 5.0)), 1/3);
  ...
  double ina_11 = params[25] * (gna_11 * pow(y[191], 3) * hs11 * cnao * y[1] * (exp((y[1] - ena) * ifactor) - 1.0)/(exp(y[1] * ifactor) - 1.0) * f * ifactor);
  //C
